...
namespace LSS_prototype.DB_CRUD
{
    public partial class DB_Manager
    {
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            return new SettingModel
                            {
                                ...
                                MwlDescriptionFilter = reader["MWL_DESCRIPTION_FILTER"].ToString()
                            };
                        }
                    }
                }
        ...
        #region [ MWL 필터 수정 담당부 ]
        public bool UpdateMwlFilter(string filter)
        {
            using (var conn = new SQLiteConnection($"Data Source={Common.DB_PATH}"))
            {
                conn.Open();
                using (var cmd = new SQLiteCommand(Query.UPDATE_MWL_FILTER, conn))
                {
                    cmd.Parameters.AddWithValue("@MwlDescriptionFilter", filter);
                    return cmd.ExecuteNonQuery() > 0;
                }
            }
        }

...
------------------------------------
충돌이 발생하거나 처리가 되지 않은 폴더->바탕화면에 폴더로 만들어졌다고 알림
import 된 날짜 폴더에-환자 파일이 포함되어 있도록
timer로 하루가 지나면 폴더는 없어지게


---------------------------------------------------------------------------
1. 원본 이동보다 복사 추천

제외 파일은 이동하지 말고 복사가 안전합니다.
원본 선택 위치의 파일을 건드리면 사용자 입장에서 위험합니다.

2. 생성일보다 폴더명 기준도 고려 가능

Directory.GetCreationTime은 환경에 따라 애매할 때가 있으니
폴더명을 yyyyMMdd_HHmmss로 만들고 파싱해서 삭제해도 좋습니다.

3. 너무 오래 보관 안 하기

1일 보관은 적절합니다.
실무적으로도 임시 폴더 느낌으로 무난합니다.


>>방식
제외 파일은 복사
같은 폴더에 reason.csv 또는 reason.txt
삭제는 앱 시작 시 / import 시작 시 1일 지난 폴더 정리

정확히 24시간 뒤 타이머 삭제보다는
앱 시작 시 / import 시작 시 오래된 폴더 정리가 더 안전

(이유)
앱이 계속 켜져 있다는 보장이 없음
타이머 기반 삭제는 누락/예외 처리 번거로움
시작 시 청소가 훨씬 단순하고 안정적

RejectedImport 하위 폴더 중
생성일이 1일 초과한 폴더 삭제

예:
오늘 import 시작할 때 또는 앱 실행할 때
오래된 RejectedImport 폴더 자동 정리

Error
파싱 실패, import 중 예외, 구조 생성 실패 등으로 처리 못 한 파일

폴더가 불필요하게 늘어나지 않음
사용자가 확인해야 하는 “문제 있는 파일”만 남음
운영 측면에서 더 깔끔함
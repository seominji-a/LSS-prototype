...
---------------------------------------------------------------------
(현재 문제 사항)
수정한 환자코드가 기존에 존재하는 코드와 같아도 그냥 저장돼버리면 안 됩니다.
E-SYNC + 생년월일 + 성별 일치인 경우만 병합 여부만 판단, 환자코드 중복 자체를 막는 역할 미존재

(수정사항)
같은 환자코드 이미 존재
-생년월일 + 성별도 일치 → 병합 후보
-그 외 → 중복 코드이므로 수정 불가
검사는 저장 후 병합 유도가 아니라, 저장 자체를 막는 방향

1) 수정 중 환자코드가 그대로면 허용
=>자기 자신 코드 그대로 저장하는 건 당연히 허용

2) 수정 중 환자코드가 다른 기존 환자와 겹치면
=>겹친 대상이 LOCAL이든 E-SYNC든 상관없이 먼저 막아야 함

3) 단, 겹친 대상과
환자코드 같고
생년월일 같고
성별 같으면
=>중복 생성 허용이 아니라 병합만 허용으로 처리

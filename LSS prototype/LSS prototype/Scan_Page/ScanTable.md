Scan 페이지 테이블 구조/Scan 페이지 처리 관련 문서
작성일: 2026-03-13
최종 수정일: 2026-03-13
작성자: 서민지
...
-언제 촬영 시작했는지
-ICG 주입 시간이 언제인지
-Accession Number가 뭔지 
-Study Instance UID가 뭔지
...
■ 권장 사항 3 (2026.03.12로 변경 완료)
DICOM/
 └─ Kang Minjae_10011 -환자 폴더(환자 이름_환자 번호)/
     └─ 20260312/ -STUDY 날짜 폴더 (하루 단위 정리 가능, 직접 탐색 가능, 날짜별 백업/이관 편리)
         └─ 202603120001/ -실제 STUDY ID 폴더
                 ├─ Kang Minjae_10011_202603120001_01_1.dcm  - INSTANCE관련 이미지 파일
                 ├─ Kang Minjae_10011_202603120001_01_2.dcm
...
■ 고려 사항
1. 하루에 새로운 검사 관련하여 새로운 촬영(study)가 계속 생성 될 경우,
촬영 완료에 대한 기준을 어떻게 잡을 건지 고려 (특정 주기마다 배치 처리되는지 명시)

-Integrated patient화면에 등록되는 시점에 영향을 미침

->시스템 상의 처리보다는 날짜 기준으로 하루가 넘어갈 때 자동으로 등록되게 하는게 나은지
->icg 설정 팝업 처리 여부에 따라 처리하는게 나은지
->한번이라도 image scan 버튼을 누르는 여부에 따라 처리하는게 나은지


2. 기존 촬영을 이어서 하던, 새로운 촬영을 하던
ICG 주입 시간 설정 팝업
-날짜의 첫 촬영에서 설정하여 DB에 한번 저장되면 표시되지 않아야함( 이 문장이 맞는지 고려)
...
-촬영 완료에 대한 기준 
바로 1회 발생 -> intergrated 화면으로 이동됨
0번째 icg 주입 시간 설정 고정

환자번호 기준 -동일 키(emrpatient 화면의 emr 환자, intergrated 화면의 local 환자 둘다 존재)
어느 하나로 촬영해도-> 같은 장소에 데이터 저장

팝업- 오늘 저장된 이미지가 있습니다 (하루가 지나면 다시 새로운 study(검사)로 인식)-완료
